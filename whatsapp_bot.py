    ...
    def do_GET(self):
        try:
            ...
            if WHATSAPP_VERIFY_TOKEN:
                ...
                logging.info("WA VERIFY: mode=%s token=%s challenge_len=%d",
                             mode, token[:10] if token else "", len(challenge))
                ...
                if mode or token or challenge:
                    logging.warning("WA verification mismatch: got token=%s expected=%s",
                                    token, WHATSAPP_VERIFY_TOKEN[:10] if WHATSAPP_VERIFY_TOKEN else "")
...
def main():
    ...
    try:
        ...
        logging.info(
            "ENV CHECK: WHATSAPP_TOKEN=%s PHONE_ID=%s VERIFY_TOKEN=%s API_VERSION=%s",
        ...
        # ====================================================
        # Keep-alive thread (prevent Render free tier sleep)
        # ====================================================
